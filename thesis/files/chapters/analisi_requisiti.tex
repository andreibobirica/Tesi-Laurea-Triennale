...
\begin{usecase}{0}{Autenticazione}
    ...
    \usecasepre{Un Utente Non Autenticato vuole accedere alle funzionalità della app}
    \usecasedesc{Questo scenario descrive un utente che vuole accedere alla funzionalità della app ma che non ne ha i permessi, per tanto deve fare la registrazione (\textit{singup}) o il \textit{login} (\textit{singin})}
...
\begin{usecase}{0.1}{Registrazione}
    \usecaseactors{Utente Non Autenticato}
    \usecasepre{Un Utente Non Autenticato NON possiede delle credenziali per poter accedere alla app}
    \usecasedesc{Questo scenario descrive un utente che non possiede credenziali di autenticazione e che non riesce ad accedere alle funzionalità della app, per tanto intraprende il processo di registrazione con cui alla fine riuscirà ad avere delle credenziali valide.}
...
\begin{usecase}{2}{Visualizzazione Serviced Oven}
    ...
    \usecasedesc{L'utente visualizza le informazioni di un determinato \textit{Serviced Oven} di cui ne ha i permessi.}
